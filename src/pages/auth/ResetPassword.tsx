...
export const ResetPassword: React.FC = () => {
    ...
    useEffect(() => {
        const handleToken = async () => {
            // Supabase recovery links usually contain tokens in the hash #access_token=...
            ...
            if (accessToken && refreshToken) {
                ...
                if (error) {
                    setError("This password reset link has expired or is already used.");
                }
            } else {
                // Check if we already have a session (recovery often signs user in automatically)
                const { data: { session } } = await supabase.auth.getSession();
                if (!session) {
                    setError("Password reset link is missing required security credentials.");
                }
            }
    ...
    const handleReset = async (e: React.FormEvent) => {
        ...
        if (error) {
            ...
            // Success: SIGN OUT and redirect to /login with success Param
                ...
                <div className="relative z-10">
                    <div className="flex flex-col items-center mb-10">
                        ...
                        <h1 className="text-3xl font-black text-white tracking-tighter">
                            {error ? 'Update Failed' : 'Reset Credentials'}
                        </h1>
                        <p className="text-slate-400 text-sm mt-2 font-medium text-center">
                            {error ? 'The recovery link is no longer valid.' : 'Finalize your identity recovery by setting a new clinical password.'}
                        </p>
                    </div>
                    ...
                    {validatingToken ? (
                        <div className="flex flex-col items-center py-10 space-y-4">
                            ...
                            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest">Verifying Recovery Tokens...</p>
                        </div>
                    ) : error ? (
                        ...
                        <form onSubmit={handleReset} className="space-y-6">
                            ...
                            >
                                {loading ? 'Securing Identity...' : 'Update Password'}
                            </button>
                        </form>
                    )}
