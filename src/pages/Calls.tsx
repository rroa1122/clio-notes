...
export function Calls() {
    ...
    const { calls, loading: isLoading, refreshCalls } = useCalls();
    const [searchTerm, setSearchTerm] = useState('');

    // Explicitly refresh calls every time the page is opened/mounted
    useEffect(() => {
        refreshCalls();
    }, []); // Only on mount to avoid loops or unnecessary fetches
    ...
    return (
        <div className="flex flex-col animate-in fade-in duration-500 max-w-7xl mx-auto w-full">
            ...
            <Card className="bg-surface border border-border/60 shadow-soft rounded-card overflow-hidden">
                <CardHeader className="pb-6 pt-8 px-4 md:px-8 border-b border-border/60">
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="hover:bg-transparent">
                                    <TableHead className="px-4 md:px-8 w-[300px]">Caller Identity</TableHead>
                                    <TableHead>Reason for Contact</TableHead>
                                    <TableHead>Date & Precision</TableHead>
                                    <TableHead>Duration</TableHead>
                                    <TableHead className="px-4 md:px-8 text-right">Actions</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {isLoading ? (
                                    Array.from({ length: 5 }).map((_, i) => (
                                        <TableRow key={i}>
                                            <TableCell colSpan={5} className="px-4 md:px-8 py-4">
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={5} className="px-4 md:px-8 py-16">
                                            <EmptyState
                                    ...
                                    filteredCalls.map((call) => (
                                        ...
                                        >
                                            ...
                                            <TableCell className="px-4 md:px-8 py-3 text-right">
