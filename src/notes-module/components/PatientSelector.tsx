    ...
    return (
        <div className="relative w-full" ref={containerRef}>
            ...
            {isOpen && (results.length > 0 || isLoading) && (
                <Card className="absolute top-[calc(100%+8px)] left-0 right-0 z-[100] shadow-[0_10px_40px_rgba(15,23,42,0.15)] rounded-xl p-1.5 border border-slate-200/60 dark:border-slate-800 bg-white dark:bg-slate-900 animate-in fade-in zoom-in-95 duration-200">
                    <div className="max-h-[300px] overflow-y-auto custom-scrollbar space-y-0.5">
                        {isLoading ? (
                            <div className="p-8 flex flex-col items-center justify-center gap-3">
                                ...
                                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{language === 'es' ? "Buscando..." : "Searching..."}</span>
                            </div>
                        ) : (
                            results.map((patient) => (
                                <button
                                    ...
                                    className="w-full flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group/item text-left"
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="size-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 group-hover/item:bg-primary/10 group-hover/item:text-primary transition-colors">
                                            <User size={14} />
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{patient.full_name}</span>
