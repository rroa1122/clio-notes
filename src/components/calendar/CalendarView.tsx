...
export function CalendarView() {
    ...
    const appointments: (Appointment & { raw: CalendarEvent })[] = events.map(event => {
        ...
        // Improved fallback chain for patient name
        const patientDisplayName = event.patient_name || event.patient_id || "Unnamed Patient";

        return {
            id: event.id,
            patient_last4: event.patient_last4 || '',
            patientName: patientDisplayName,
            visitType: (event.visit_type as any) || "Consultation",
    ...
    const handleConfirmEvent = async () => {
        if (!selectedEvent || selectedEvent.approval_status === 'CONFIRMED') return;
        await calendarService.confirmEvent(selectedEvent.id, "Staff Member"); // In a real app, use active user name
        fetchEvents();
    };
                                ...
                                {weekDays.map((day) => {
                                    ...
                                    return (
                                        <div key={dayStr} className={cn(
                                            "flex-1 border-r border-slate-200/70 relative min-w-[100px]",
