...
async function main() {
    ...
    const tempServiceSnapshot = await page.evaluate(() => {
        const vm = (document.querySelector('#site') || document.querySelector('#app') || document.querySelector('.main-content'))?.__vue__ || window.vuem;
        return {
            temp_service: JSON.parse(JSON.stringify(vm?.temp_service || {})),
            timeInputs: {
                start: document.querySelector('#temp_service_hora_inicio')?.value,
                end: document.querySelector('#temp_service_hora_fin')?.value
            }
        };
    });

    console.log('TEMP SERVICE SNAPSHOT:\n', JSON.stringify(tempServiceSnapshot, null, 2));
    await context.close();
}
