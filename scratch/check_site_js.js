...
async function main() {
    ...
    await page.goto('https://www.amexzone.com/back/js/site.js?v=28', { waitUntil: 'load' });
    await page.waitForTimeout(2000);

    const txt = await page.evaluate(() => document.body.innerText);
    console.log('SITE.JS SIZE:', txt.length);
    const idx = txt.indexOf('saveCitaServicio');
    if (idx !== -1) {
        console.log('FOUND saveCitaServicio IN SITE.JS:\n', txt.slice(idx - 100, idx + 2000));
    } else {
        console.log('saveCitaServicio not in site.js');
    }
