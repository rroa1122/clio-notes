...
if row:
    ...
    for node in nodes:
        if node.get('name') == 'OpenAI Responses API' and node.get('type') == 'n8n-nodes-base.httpRequest':
            ...
            if '"model": "gpt-5-mini"' in body_str:
                params['jsonBody'] = body_str.replace('"model": "gpt-5-mini"', '"model": "gpt-4o-mini"')
                modified = True
                print("Updated model in node OpenAI Responses API parameters to gpt-4o-mini")
            elif '"model":"gpt-5-mini"' in body_str:
                params['jsonBody'] = body_str.replace('"model":"gpt-5-mini"', '"model":"gpt-4o-mini"')
                modified = True
                print("Updated model in node OpenAI Responses API parameters to gpt-4o-mini (no space)")
    ...
    if modified:
        ...
        print("Model gpt-5-mini was not found in OpenAI Responses API parameters.")
else:
