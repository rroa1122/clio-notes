...
async function main() {
    ...
    const saveServiceFn = await page.evaluate(() => {
        const vm = (document.querySelector('#site') || document.querySelector('#app') || document.querySelector('.main-content'))?.__vue__;
        if (!vm) return 'no vm';
        
        // Find any method related to saving service
        const methods = vm.$options?.methods || {};
        const fnNames = Object.keys(methods).filter(k => /service|servicio|save|guardar/i.test(k));
        const res = {};
        for (const name of fnNames) {
            res[name] = methods[name].toString().slice(0, 500);
        }
        return res;
    });

    console.log('SAVE SERVICE METHODS:\n', JSON.stringify(saveServiceFn, null, 2));
    await context.close();
}
