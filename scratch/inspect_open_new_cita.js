...
async function main() {
    ...
    const functionsInfo = await page.evaluate(() => {
        return {
            openNewCita: typeof window.openNewCita === 'function' ? window.openNewCita.toString() : 'no openNewCita',
            openNewCitaPaciente: typeof window.openNewCitaPaciente === 'function' ? window.openNewCitaPaciente.toString() : 'no openNewCitaPaciente',
            saveCita: typeof window.saveCita === 'function' ? window.saveCita.toString() : 'no saveCita',
            guardarCita: typeof window.guardarCita === 'function' ? window.guardarCita.toString() : 'no guardarCita'
        };
    });
