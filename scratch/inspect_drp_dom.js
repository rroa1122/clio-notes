...
async function main() {
    ...
    // Click date input to open daterangepicker
    const dateInp = page.locator('#cita_fecha_hora');
    await dateInp.click();
    await page.waitForTimeout(1500);

    const drpDom = await page.evaluate(() => {
        const picker = document.querySelector('.daterangepicker:not([style*="display: none"]), .daterangepicker');
        if (!picker) return 'no daterangepicker';
        return {
            classes: picker.className,
            selects: Array.from(picker.querySelectorAll('select')).map(s => ({
                className: s.className,
                value: s.value,
                options: Array.from(s.options).map(o => o.text)
            })),
            inputs: Array.from(picker.querySelectorAll('input')).map(i => ({
                className: i.className,
                value: i.value
            })),
            buttons: Array.from(picker.querySelectorAll('button')).map(b => b.innerText)
        };
    });

    console.log('DATERANGEPICKER DOM:\n', JSON.stringify(drpDom, null, 2));
    await context.close();
}
