...
async function main() {
    ...
    async function doSearch(query) {
        ...
        const hasNerci = await page.evaluate(() => document.body.innerText.includes('Nerci'));
        const rowsCount = await page.evaluate(() => document.querySelectorAll('table tbody tr').length);
        console.log(`Search query "${query}": hasNerci=${hasNerci}, rowsCount=${rowsCount}`);
    }
    ...
    try {
        ...
        await doSearch('Nerci');
        await doSearch('Perurena');
        await doSearch('Nerci Perurena');
        await doSearch('Nerci Perurena Artiles');
