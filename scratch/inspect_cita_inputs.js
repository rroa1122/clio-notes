...
async function main() {
    ...
    const inputsInfo = await page.evaluate(() => {
        const modals = Array.from(document.querySelectorAll('.modal.show, #gestion_cita_modal, #new_cita_modal, div[role="dialog"]'));
        return modals.map(m => ({
            modalId: m.id,
            modalClass: m.className,
            inputs: Array.from(m.querySelectorAll('input, select, textarea')).map(i => ({
                id: i.id,
                name: i.name,
                type: i.type,
                value: i.value,
