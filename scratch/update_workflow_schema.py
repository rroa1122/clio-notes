...
new_prompt = (
    ...
    "   - Identifica si el proveedor de la visita es un proveedor psiquiátrico o de salud mental (ej. Psiquiatra, APRN de salud mental, psicólogo, clínica de mental health). Si es así, pon sus datos en 'psychiatrist' y deja 'pcp' vacío o con su PCP habitual si se menciona.\n"
    "   - Si es un proveedor de medicina familiar/general (PCP) (ej. MD general, clínica familiar), pon sus datos en 'pcp' y deja 'psychiatrist' vacío.\n"
        ...
        try:
            ...
            if 'input' in body and len(body['input']) > 1:
                ...
                if 'content' in msg and len(msg['content']) > 0:
                    msg['content'][0]['text'] = new_prompt
                    
                    # Update structured rules for array items just in case the system uses them
                    # In our case we are not using the old array rules section so we can keep it clean.
                    
                    params['jsonBody'] = ('=' if has_equals else '') + json.dumps(body)
                    modified = True
                    print("Prompt schema successfully updated in OpenAI Responses API node!")
