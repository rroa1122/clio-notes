...
async function main() {
    ...
    const nuevaCitaBtnInfo = await page.evaluate(() => {
        const btns = Array.from(document.querySelectorAll('button, a')).filter(b => /Nueva Cita|New Appointment/i.test(b.innerText || ''));
        return btns.map(b => ({
            tag: b.tagName,
            id: b.id,
    ...
    console.log('NUEVA CITA BTN INFO:\n', JSON.stringify(nuevaCitaBtnInfo, null, 2));
    await context.close();
}
