...
async function main() {
    ...
    const instances = await page.evaluate(() => {
        const els = Array.from(document.querySelectorAll('*')).filter(e => e.__vue__);
        return els.map(e => ({
            tag: e.tagName,
            id: e.id,
            className: e.className,
            keys: Object.keys(e.__vue__.$data || {}).slice(0, 10),
            citas: e.__vue__.citas ? Object.keys(e.__vue__.citas) : null
        }));
    });

    console.log('ALL VUE INSTANCES ON PAGE:\n', JSON.stringify(instances, null, 2));
    await context.close();
}
