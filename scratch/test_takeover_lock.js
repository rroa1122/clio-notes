...
async function main() {
    ...
    await page.goto('https://www.amexzone.com/attend/appointment/1585872', { waitUntil: 'load' });
    ...
    const beforeTakeover = await page.evaluate(() => {
        ...
        return {
            hasLockOverlay: !!lockEl,
            edit_lock: vm?.edit_lock,
            tab_token: vm?.tab_token
        };
    });
    console.log('BEFORE TAKEOVER:\n', JSON.stringify(beforeTakeover, null, 2));

    // Execute takeover
    await page.evaluate(() => {
        const vm = (document.querySelector('#site') || document.querySelector('#app') || document.querySelector('.main-content'))?.__vue__ || window.vuem;
        if (vm) {
            if (typeof vm.takeoverEditLock === 'function') {
                vm.takeoverEditLock();
            }
            if (typeof vm.editLockFailOpen === 'function') {
                vm.editLockFailOpen();
            }
        }
    });
    ...
    const afterTakeover = await page.evaluate(() => {
        const vm = (document.querySelector('#site') || document.querySelector('#app') || document.querySelector('.main-content'))?.__vue__ || window.vuem;
        const lockEl = Array.from(document.querySelectorAll('div, p, span')).find(el => /otra pestaña o dispositivo/i.test(el.innerText || ''));
        return {
            hasLockOverlay: !!lockEl,
    ...
    console.log('AFTER TAKEOVER:\n', JSON.stringify(afterTakeover, null, 2));
