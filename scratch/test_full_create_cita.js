...
async function main() {
    ...
    await page.evaluate(() => {
        ...
        // 2. Worker
        const cmSel = jQ('#cita_trabajador_id');
        if (cmSel.length) cmSel.val('473').trigger('change');

        // 3. Duration
        const durInp = jQ('#cita_duracion_mins');
        if (durInp.length) durInp.val(60).trigger('change').trigger('input');

        // 4. Place: 12 - Home -> home_visit
        const placeSel = jQ('#cita_place');
        if (placeSel.length) placeSel.val('home_visit').trigger('change');

        // 5. Date & Time
        const inp = jQ('#cita_fecha_hora');
        ...
        inp.val(formatted).trigger('change').trigger('input');
    });
