...
if row:
    ...
    for node in nodes:
        if node.get('name') == 'OpenAI Responses API' and node.get('type') == 'n8n-nodes-base.httpRequest':
            ...
            if '"model": "gpt-4o-mini"' in body_str:
                params['jsonBody'] = body_str.replace('"model": "gpt-4o-mini"', '"model": "gpt-5.4"')
                modified = True
                print("Updated model in node OpenAI Responses API parameters to gpt-5.4")
            elif '"model":"gpt-4o-mini"' in body_str:
                params['jsonBody'] = body_str.replace('"model":"gpt-4o-mini"', '"model":"gpt-5.4"')
                modified = True
                print("Updated model in node OpenAI Responses API parameters to gpt-5.4 (no space)")
    ...
    if modified:
        ...
        print("Model gpt-4o-mini was not found in OpenAI Responses API parameters.")
else:
